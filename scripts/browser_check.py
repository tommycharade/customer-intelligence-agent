    ...
    with tempfile.TemporaryDirectory(prefix="customer-intelligence-ui-") as directory:
        ...
        try:
            with sync_playwright() as playwright:
                ...
                credits = page.get_by_role("heading", name="About and credits", exact=True).locator("..")
                expect(credits).to_contain_text("software developed by UncleCode")
                credits.scroll_into_view_if_needed()
                credits.screenshot(path=str(artifacts / "credits-desktop.png"))
                page.set_viewport_size({"width": 390, "height": 844})
                credits.scroll_into_view_if_needed()
                assert page.evaluate("document.documentElement.scrollWidth <= innerWidth")
                credits.screenshot(path=str(artifacts / "credits-mobile.png"))
                page.set_viewport_size({"width": 1440, "height": 1000})
                notices = page.request.get("http://127.0.0.1:8878/third-party-notices.txt")
                assert notices.status == 200 and "ISC License" in notices.text()
                assert "Copyright (c) Meta Platforms, Inc." in notices.text()
                ...
                expect(
                    page.get_by_text("Synthetic fixture: model usage display only.", exact=True)
                ).to_be_visible()
                page.get_by_text("Models and spending", exact=True).click()
